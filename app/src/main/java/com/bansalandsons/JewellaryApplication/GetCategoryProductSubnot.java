...
import com.bansalandsons.JewellaryApplication.Adapterclasses.AdpterNoSub;
import com.bansalandsons.JewellaryApplication.pojoclasses.POJOGETNOSUB;
...
public class GetCategoryProductSubnot extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvnoproduct = findViewById(R.id.noProductsTextView);
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            if (response.getInt("status") == 0) { // Check status = 0
                                ...
                                if (pojogetnosubs.isEmpty()) {
                                    tvnoproduct.setVisibility(View.VISIBLE); // Show "No products" text
                                    rvList.setVisibility(View.GONE); // Hide RecyclerView
                                } else {
                                    tvnoproduct.setVisibility(View.GONE); // Hide "No products" text
                                    rvList.setVisibility(View.VISIBLE); // Show RecyclerView
